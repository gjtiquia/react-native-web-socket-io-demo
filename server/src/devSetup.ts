...
// This code is run by npm commands
export default async function main() {
    // Gets all the remote IPs for the development server machine. This gets the same value as Expo's development server.
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        fs.writeFile(FILE_PATH, FILE_CONTENTS, (err) => {
            if (err) {
                ...
                console.log(`Successfully wrote current Host IP and PORT to ${FILE_PATH}`);
                resolve();
            }
        })
